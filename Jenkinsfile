pipeline {
    ...
    environment {
        ...
        JAVA_HOME = '/Library/Java/JavaVirtualMachines/jdk-21.0.7.jdk/Contents/Home'
        PATH = "/Library/Java/JavaVirtualMachines/jdk-21.0.7.jdk/Contents/Home/bin:/usr/local/bin:/opt/homebrew/bin:${PATH}"
    }
            ...
            steps {
                script {
                    if (params.IS_ROLLBACK) {
                        ...
                        sh "docker run -d --name weather-app-container -p 8081:8081 -e WEATHER_API_KEY=${WEATHER_API_KEY} ${DOCKER_IMAGE}:${params.ROLLBACK_TO}"
                    } else {
                        echo "Deploying build #${BUILD_NUMBER}"
                        sh "docker stop weather-app-container || true"
                        sh "docker rm weather-app-container || true"
                        sh "docker run -d --name weather-app-container -p 8081:8081 -e WEATHER_API_KEY=${WEATHER_API_KEY} ${DOCKER_IMAGE}:${DOCKER_TAG}"
                    }
                }
            }
            post {
